...
/**
 * Identity function that returns data as-is
 *
 * Previously this function unwrapped single-item arrays that were generated by
 * R's htmlwidgets:::toJSON2() serialization. Since we've switched to jsonlite::toJSON()
 * with proper data conversion (using as.list() instead of split()), the data no longer
 * contains unwanted array wrapping.
 *
 * This function is kept as a no-op for backwards compatibility with user-defined
 * event handlers that may call flattenData(). It can be safely removed in a future
 * major version.
 *
