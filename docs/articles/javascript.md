...
output binding with direct JSON serialization, giving you clean,
predictable output and full access to Tabulator’s extensive API.

### Why Tabulator Works Well with R

Tabulator is a JavaScript library that uses a standard JSON object to
render interactive tables. Since the Tabulator specification is
essentially a named list (JSON object), it makes Tabulator an ideal
candidate for an R wrapper. R’s data structures serialize cleanly to the
exact format Tabulator expects, requiring minimal transformation.

Here’s an example from the [Tabulator
documentation](http://tabulator.info/docs/5.5/quickstart) showing a
typical table configuration:
...
Notice the one-to-one correspondence: - JSON objects
[`{}`](https://rdrr.io/r/base/Paren.html) → R named lists
[`list()`](https://rdrr.io/r/base/list.html) - JSON arrays `[]` → R
lists [`list()`](https://rdrr.io/r/base/list.html) - JSON booleans
`true/false` → R logicals `TRUE/FALSE` - JSON strings and numbers remain
the same

This natural mapping means tabulatoR can pass R configurations directly
to Tabulator with minimal overhead.
